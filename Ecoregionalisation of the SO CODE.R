...
MRTclust <- as.data.frame(spech.mvpart.g)
MRTclust <- mutate(MRTclust, data = seq(n()))
...
Data.scores = right_join(Data.scores, MRTclust)
datanMDS$stress <- round(datanMDS$stress, 4)
Data.scores$MRTClust <- as.factor(Data.scores$MRTClust)


plot = ggplot() +
  geom_point(data=Data.scores, aes(x=NMDS1, y=NMDS2, shape = Zone, colour = MRTClust), size = 3) +
